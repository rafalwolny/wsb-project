#!/bin/bash
echo "STEP 1 - read config files"
export $(grep -v '^#' .config | xargs)
export $(grep -v '^#' .env | xargs)


echo "STEP 2 - copy files for upload to S3"
# copy
...
# adjust
sed -i "s;http://localhost:3000;https://${API_SUBDOMAIN}.${DOMAIN_NAME};g" $CDK_DEPLOYMENT_DIR/frontend/index.js
sed -i "s/TAG/$DOCKER_TAG/g" $CDK_DEPLOYMENT_DIR/backend/wsb/docker-compose-aws.yml


echo "STEP 3 - execute cdk deploy"
echo "Deploying CloudFormation templates..."
cd cdk
cdk deploy wsb-frontend-stack wsb-backend-stack --require-approval never
